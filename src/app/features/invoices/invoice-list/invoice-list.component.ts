    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
          ...
          <p class="mt-2 text-sm text-gray-700">Manage all invoices for appointments.</p>
        </div>
        <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
          <a routerLink="/invoices/new" class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
            Create Invoice
          </a>
        </div>
      </div>
            ...
            <div class="flex items-center">
              ...
              <div class="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd class="text-lg font-semibold text-gray-900">{{ invoices.length }}</dd>
                </dl>
              </div>
            </div>
          </div>
            ...
            <div class="flex items-center">
              ...
              <div class="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd class="text-lg font-semibold text-green-600">{{ totalAmount | currency }}</dd>
                </dl>
              </div>
            </div>
          </div>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  <tr *ngIf="filteredInvoices.length === 0">
                    <td colspan="5" class="py-12 text-center text-sm text-gray-500">
                      ...
                      <a routerLink="/invoices/new" class="mt-2 inline-block text-indigo-600 hover:text-indigo-500">Create an invoice</a>
                    </td>
                  </tr>
                  <tr *ngFor="let invoice of filteredInvoices" class="hover:bg-gray-50">
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                      INV-{{ invoice.invoice_id ? invoice.invoice_id.toString().padStart(5, '0') : '00000' }}
                    </td>
                    ...
                    <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                      <a [routerLink]="['/invoices', invoice.invoice_id, 'edit']" class="text-indigo-600 hover:text-indigo-900 mr-4">Edit</a>
                      <button (click)="deleteInvoice(invoice)" class="text-red-600 hover:text-red-900" [disabled]="deletingId === invoice.invoice_id">
                        {{ deletingId === invoice.invoice_id ? 'Deleting...' : 'Delete' }}
                      </button>
                    </td>
                  </tr>
                </tbody>
      ...
      <!-- Summary -->
      <div *ngIf="!isLoading && filteredInvoices.length > 0" class="mt-4 text-sm text-gray-500">
        Showing {{ filteredInvoices.length }} of {{ invoices.length }} invoices
      </div>
    </div>

    <!-- Delete Confirmation Modal -->
    <div *ngIf="invoiceToDelete" class="fixed inset-0 z-50 overflow-y-auto">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2">
                <p class="text-sm text-gray-500">Are you sure you want to delete Invoice #INV-{{ invoiceToDelete.invoice_id ? invoiceToDelete.invoice_id.toString().padStart(5, '0') : '00000' }}? This action cannot be undone.</p>
              </div>
            </div>
          </div>
...
export class InvoiceListComponent implements OnInit {
  ...
  loadInvoices(): void {
    ...
    forkJoin({
      ...
      next: ({ invoices, clients }) => {
        ...
        this.clientMap = new Map(clients.map(c => [c.client_id!, c]));
  ...
  onFilter(): void {
    ...
    if (this.searchQuery.trim()) {
      ...
      result = result.filter(invoice => {
        const clientName = this.getClientName(invoice.client_id).toLowerCase();
        return invoice.invoice_id?.toString().includes(query) ||
  ...
  confirmDelete(): void {
    if (!this.invoiceToDelete?.invoice_id) return;

    this.deletingId = this.invoiceToDelete.invoice_id;

    this.invoiceService.deleteInvoice(this.invoiceToDelete.invoice_id).subscribe({
      next: () => {
        this.invoices = this.invoices.filter(i => i.invoice_id !== this.deletingId);
        this.filteredInvoices = this.filteredInvoices.filter(i => i.invoice_id !== this.deletingId);
        this.invoiceToDelete = null;
